...
class Network(object):
    ...
    def conv(self,
        ...
        with tf.variable_scope(name) as scope:
            ...
            weight_decay = tf.multiply(tf.nn.l2_loss(kernel), 5e-4,
                                       name='weight_loss')
    ...
    def avg_pool(self, input, k_h, k_w, s_h, s_w, name, padding=DEFAULT_PADDING):
        ...
        # a = tf.cast(tf.Variable([shape[1], shape[2]], trainable=False), dtype=tf.float32)
        # b = tf.Variable([factor, factor], trainable=False, dtype=tf.float32)

        # k_h = tf.Variable([k_h], trainable=False)
        # k_w = tf.Variable(k_w, trainable=False)
        # s_h = tf.Variable(s_h, trainable=False)
        # s_w = tf.Variable(s_h, trainable=False)
        # one = tf.constant(1)
        # a = tf.cast(tf.Variable([shape[1], shape[2]], trainable=False), dtype=tf.int32)
        # stride = tf.Variable([1, s_h, s_w, 1], trainable=False, dtype=tf.int32)
    ...
    def fc(self, input, num_out, name, relu=True):
        with tf.variable_scope(name) as scope:
            ...
            weight_decay = tf.multiply(tf.nn.l2_loss(weights), 5e-4,
                                       name='weight_loss')
            tf.add_to_collection(tf.GraphKeys.REGULARIZATION_LOSSES,
                                 weight_decay)
    ...
    def interp(self, input, s_factor=1, z_factor=1, name=None):
        ...
        ori_h = ori_h * s_factor * z_factor
        ori_w = ori_w * s_factor * z_factor
